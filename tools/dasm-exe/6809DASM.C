...
/* - help (small) included */
/* - compatability switch included for comaptability to motorala's public */
...
char *fulladdr="0x%04X";
char *shortaddr="0x%02X";
...
//   address&=0xff;
  ...
  if(!strlen(labtemp)&&!lab2)
  {
   ...
   else
   {
    if ((address<128)&&(address>-129))
    {
     sprintf(labtemp,"-0x%02X",-address);
    }
    else
    {
     sprintf(labtemp,"-0x%04X",-address);
    }
   }
  }
 }
  ...
  case IND:         /* indirect- many flavors */
   ...
   if (as9_comp == YES)
   {
    ...
    if (s_helper!=TRUE)
    {
     ...
     {
      sprintf(out2, ", 0x%02X" , (UBYTE) operandarray[i]);
      strcat(str, out2);
     }
     if (s_helper==SHOW_IT)
     {
...
*/
    ...
    if(pb==0x8c)
     ...
     sprintf(out2,"0x%02X,%s",offset,regs[reg]);
    else
     sprintf(out2,"-0x%02X,%s",-offset,regs[reg]);
    strcat(str,out2);
    if(pb==0x8c)
    {
     sprintf(out2," ; (0x%04X)",offset+count);
     strcat(str,out2);
    }
   }
...
/*    ...*/
    ...
    if(pb==0x9f)
     ...
     sprintf(out2,"0x%04X,%s",offset,regs[reg]);
    else
     sprintf(out2,"-0x%04X,%s",offset,regs[reg]);
    strcat(str,out2);
    if(pb==0x8d)
...
int dumpdata(int opcode, int *curdata, int numdata, FILE *fp, int *curcomm, int numcomm, int *curcommline, int numcommline)
{
 ...
 {                                                                             /* print the data */
  ...
  if ((dataarray[*curdata].type == DATA) || ((dataarray[*curdata].type == WTEXT)&&(as9_comp == YES)) )
  {
   ...
   {
    if (numchars==0)
     sprintf(out2, " 0x%02X", getbyte(fp));                                        /* hex */
    else
     sprintf(out2, ", 0x%02X", getbyte(fp));                                        /* hex */
   }
   else
  ...
  while((opcode=getbyte(fp))!=EOF)
  {
   ...
   if ((as9_comp==YES)&&(commentlinenumber==YES))
   {
    for(sp=strlen(out);sp<LINENUMTAB;sp++)
     strcat(out," ");
    sprintf(out2, " <0x%04X>", this_line_adress);
    strcat(out,out2);
   }
